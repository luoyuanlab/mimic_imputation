evalPtTensorImpImportTrTe <- function(timp, tgt, ci, naidx, V1.raw=NULL, V2.raw=NULL, V1.sd=NULL, V2.sd=NULL, metric='RMSE range norm', err.out=T, trte='tr', iter=1, fncf='mimicConfig.R') {
    ...
    for(test in tests){
        ...
        cat(sprintf('%s n%s\n', test, metric))
        print(quantile(nrmse.v[[test]], qtl, na.rm=T), digits=3)
    }
